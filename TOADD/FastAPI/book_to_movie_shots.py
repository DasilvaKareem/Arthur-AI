...
def generate_book_to_movie(pdf_file_path, video_length_minutes, style="anime"):
    """
    Generate movie shots based on a book PDF and desired video length.
    ...
    try:
        ...
        print(f"Successfully uploaded PDF: {pdf_file_path}")
    ...
    contents = [
        types.Content(
            ...
            parts=[
                ...
                types.Part.from_text(text=f"""Create a {style}-style movie adaptation of this book with {num_shots} shots (5 seconds per shot).
...
Organize the shots into logical scenes from the book.
Keep the total number of shots to exactly {num_shots}.
Make sure every shot captures a crucial moment from the book.
...
"""),
    ...
    print(f"\nGenerating {style}-style movie shots from the book. This may take a few minutes...")
...
if __name__ == "__main__":
    # Default settings
    default_pdf = "chapter1.pdf"
    default_length = 1.0  # 1 minute
    default_style = "anime"
    
    print(f"Default settings: {default_pdf}, {default_length} minute, {default_style} style")
    ...
    if use_defaults == 'y':
        ...
        if os.path.exists(default_pdf):
            generate_book_to_movie(default_pdf, default_length, default_style)
        else:
            print(f"Error: Default PDF '{default_pdf}' not found. Please provide a valid file path.")
            pdf_path = input("Enter the path to your book PDF file: ")
            generate_book_to_movie(pdf_path, default_length, default_style)
    else:
        # Check if processing existing JSON or generating new
        option = input("Choose an option:\n1. Generate new shots from book PDF\n2. Process existing JSON file\nEnter choice (1 or 2): ")
        
        if option == "1":
            # Get the PDF file path
            pdf_path = input(f"Enter the path to your book PDF file (default: {default_pdf}): ") or default_pdf
            ...
            # Generate shots from the book
            generate_book_to_movie(pdf_path, video_length, style)
